...
from src.model.repository.cursoRepository import CursoRepository
from src.model.cursoModel import CursoRequest, CursoResponse
...
def create(request: CursoRequest, db: Session = Depends(get_db)):
    curso = CursoRepository.save(db, CursoEntity(**request.dict()))
...
def update(id: int, request: CursoRequest, db: Session = Depends(get_db)):
    ...
    curso = CursoRepository.save(db, CursoEntity(id=id, **request.dict()))
    return CursoResponse.from_orm(curso)
